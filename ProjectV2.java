...
public class ProjectV2{
  ...
  public static void main(String[] args) throws Exception{
        ...
        double[] arrayP = new double[array1.length];
        int k = 0;
  ...
  for(int i=0; i < GroupList.size(); i++){
    ...
    for(int a = 1; a <= Group2.size(); a++){
      if (a == 1){
        ...
        while((line = br.readLine()) != null){
          ...
          Group2Data.add(CSGroup2Data);
      // System.out.print(Group2Data + " | ");
        }
        */
        for(int b = 1; b < Group2.size(); b++){
          String filename = (Group2.get(b));
          FileReader textFileReader = new FileReader(filename);
          BufferedReader bufReader = new BufferedReader(textFileReader);
    ...
    //Reading the Computer Science Program Group
      ...
      for(int a = 1; a <= Group3.size(); a++){
        if (a == 1){
          for(int b = 1; b < Group3.size(); b++){
          String filename = (Group3.get(b));
          FileReader textFileReader = new FileReader(filename);
          BufferedReader bufReader = new BufferedReader(textFileReader);
